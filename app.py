...
with col3:
    st.metric(
        label="Stations w Empty Docks", value=len(data[data["num_docks_available"] > 0])
...
with st.sidebar:
    ...
    if bike_method == "Rent":
        ...
        )  # Multi-select box for bike types
        ...
        findmeabike = st.button(
            "Find me a bike!", type="primary"
        )  # Button to find a bike
        if findmeabike:
            if input_street != "":
                iamhere = geocode(
                    input_street + " " + input_city + " " + input_country
                )  # Geocode the input address
                if iamhere == "":
                    st.subheader(
                        ":red[Input address not valid!]"
                    )  # Display error if address is invalid
            else:
                st.subheader(
                    ":red[Please input your location.]"
                )  # Prompt user to input location if missing
    elif bike_method == "Return":
        st.subheader("Where are you located?")
        input_street_return = st.text_input(
            "Street", ""
        )  # Text input for street for return
        input_city_return = st.text_input(
            "City", "Toronto"
        )  # Text input for city for return
        input_country_return = st.text_input(
            "Country", "Canada"
        )  # Text input for country for return
        findmeadock = st.button(
            "Find me a dock!", type="primary"
        )  # Button to find a dock
        if findmeadock:
